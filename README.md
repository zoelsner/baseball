...
- **`sandlot_api.py` / `sandlot_cron.py`** — Sandlot V1 web app. Railway-ready FastAPI service with Postgres snapshots, manual refresh, twice-daily scheduled Railway scrape, player detail sheets, MLB headshots/game logs/media, optional background profile warming, cached Skipper takes, and the deterministic read-only [Win This Week decision engine](docs/win-this-week.md). See [`docs/sandlot-railway-v1.md`](docs/sandlot-railway-v1.md).
