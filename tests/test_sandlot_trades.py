...
class TradeCounterTests(unittest.TestCase):
    ...
    def test_manual_review_turns_blocked_offer_into_read_only_decision_brief(self):
        ...
        self.assertTrue(review["skipper_prompt"].startswith("Sandlot trade-analysis evidence:"))
        self.assertIn("Weekly impact; Rest-of-season; Dynasty; Roster fit; Replacement value; Counteroffer", review["skipper_prompt"])
        self.assertIn("web-verified facts with source links", review["skipper_prompt"])
        self.assertIn("ACCEPT, REJECT, COUNTER, or HOLD", review["skipper_prompt"])
