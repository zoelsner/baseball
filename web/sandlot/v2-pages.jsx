...
function v2ProjectionInfo(projection) {
  ...
  const color = pct >= 60 ? V2.ok : pct >= 40 ? V2.warn : V2.bad;
  return {
    pct,
    color,
  ...
  return (
    <div style={{ width:66, height:66, position:'relative', flexShrink:0 }}>
      ...
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
        <div style={{ color:info.color, fontSize:17, lineHeight:1, fontWeight:800, fontFamily:V2.fontMono }}>{info.pct}</div>
        <div style={{ marginTop:3, color:V2.muted, fontSize:7.5, lineHeight:1, fontWeight:800, letterSpacing:'0.04em' }}>% TO WIN</div>
      </div>
    </div>
  );
}
...
function V2Today({ model, sync, onRefresh, onNav, onPlayer }) {
  ...
  const showProjection = projectionInfo && !(projectionInfo.complete && matchup?.margin !== 0);
