...
- Single-user app — Sandlot read routes are mostly unauthenticated by design. `/api/refresh` has an optional `SANDLOT_REFRESH_TOKEN` guard. Execution-control routes are disabled by default and require separate hashed owner and runner bearer credentials; never expose either plaintext credential to the browser bundle.
